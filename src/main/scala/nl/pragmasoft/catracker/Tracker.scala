...
object Trackers extends LazyLogging {
  ...
  def toClient(deviceId: DeviceId, connectionId: ConnectionId)
    (implicit concurrent: Concurrent[IO]): Stream[IO, WebSocketFrame] = {
  ...
  def fromClient(deviceId: DeviceId, connectionId: ConnectionId)
    (wsfStream: Stream[IO, WebSocketFrame]): Stream[IO, Unit] =
        wsfStream
          .collect {
            case Text(text, _) => {
              (for {
    ...
    Behaviors.setup { context =>
      ...
      Behaviors.receiveMessage {
        ...
        case ServerMessage(deviceId, payload)          =>
            activeConnections.get(deviceId).foreach( d =>
              d.foreach {
                case (_, topic) =>
  ...
  case class State(deviceId: DeviceId, positions: List[StoredPosition]) {
    def messagesOnIncomingPosition(incoming: StoredPosition): List[ServerMessage] = {
      ...
      (positions match {
        ...
        case p :: _ if
        incoming.positionFix &&
          (!p.positionFix || (incoming.recorded - p.recorded) > RestartTrackingAfterMillis) =>
    ...
    EventSourcedBehavior[StateCommand, Event, State](
      ...
      .receiveSignal {
         case (state, RecoveryCompleted)
           if state.positions.nonEmpty => parent ! ServerMessage(device, Path(state.positions))
      }
