...
class PositionDatabase(transactor: Transactor[IO]) extends PositionRepository[IO] {
  ...
  def findForDevice(deviceSerial: String): IO[List[StoredPosition]] =
    sql"SELECT id, recorded, app, deviceType, deviceSerial, latitude, longitude, positionFix, bestGateway, bestSNR, battery, temperature, counter FROM positions WHERE deviceSerial=$deviceSerial and positionFix=1 and latitude<>0 and longitude<>0 order by recorded desc limit 100"
