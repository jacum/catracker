...
object Database extends LazyLogging {
  ...
  def initialize(transactor: HikariTransactor[IO]): IO[Unit] = transactor.configure { dataSource =>
    IO {
...
class PositionDatabase(transactor: Transactor[IO]) extends PositionRepository[IO] {
  ...
  def add(p: StoredPosition): IO[Unit] =
     sql"INSERT INTO positions (recorded, app, deviceType, deviceSerial, latitude, longitude, positionFix, bestGateway, bestSNR, battery, temperature, counter) VALUES (${p.recorded}, ${p.app}, ${p.deviceType}, ${p.deviceSerial}, ${p.latitude}, ${p.longitude}, ${p.positionFix}, ${p.bestGateway}, ${p.bestSNR}, ${p.battery}, ${p.temperature}, ${p.counter})"
            .update
  ...
  def findForDevice(deviceSerial: String): IO[List[StoredPosition]] =
    sql"SELECT id, recorded, app, deviceType, deviceSerial, latitude, longitude, positionFix, bestGateway, bestSNR, battery, temperature, counter FROM positions WHERE deviceSerial=$deviceSerial order by recorded desc"
